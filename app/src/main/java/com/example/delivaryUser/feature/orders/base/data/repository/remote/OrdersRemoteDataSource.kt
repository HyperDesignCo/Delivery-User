...
class OrdersRemoteDataSource(val provider: IRemoteDataSourceProvider) : IOrdersRemoteDataSource {
    ...
    override suspend fun getOrderById(
        token: String,
        id: Int,
    ): OrderDetailsDto = provider.get(
        headers = mapOf("Authorization" to "Bearer $token"),
        params = mapOf("id" to id),
        endpoint = ORDER_DETAILS,
        serializer = OrderDetailsDto.serializer(),
    )
